  ...
  >
    ...
    >
      <template #default="{ row }">
        <p class="text-center">
          {{ formatterPubday(row).slice(0, 10) }}
        </p>
        <p class="text-center">
          {{ formatterPubday(row).slice(10) + '00:00:00' }}
        </p>
      </template>
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // },
    formatterPubday(row) {
      // console.log('row, column, cellValue, index', row)
      return timeFormat(row.pubday, true, 'YYYY-MM-DD')
    },
  },
  // mounted() {
  //   this.mockData()
